    ...
    try {
      const [balance, staked, currentAllowance, userStakeInfo] = await Promise.all([
        ...
        publicClient.readContract({
          ...
          args: [userAddress as `0x${string}`, DASH_STAKING],
        }),
        publicClient.readContract({
          address: DASH_STAKING,
          abi: stakingABI,
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        ...
        <div className="bg-slate-800/50 rounded-lg p-4">
          ...
          <p className="text-lg font-semibold text-white">
            {isLoading ? '...' : `${Number(formatUnits(totalStaked, 18)).toFixed(2)} DASH`}
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            ...
            <p className="font-semibold text-white">
              {isLoading ? '...' : `${Number(formatUnits(userStakedAmount, 18)).toFixed(2)} DASH`}
            </p>
          </div>
          <div>
          ...
          <div>
            ...
            <p className="font-semibold text-white">
              {isLoading ? '...' : `${Number(formatUnits(userBalance, 18)).toFixed(2)} DASH`}
            </p>
          </div>
        </div>
      </div>
